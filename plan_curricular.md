...
```mermaid
graph TD
    A[Fundamentos Algebraicos] --> B[Expresiones Algebraicas]
    A --> C[Ecuaciones Lineales]
    B --> D[Factorización]
    C --> E[Sistemas de Ecuaciones]
    D --> F[Ecuaciones Cuadráticas]
    E --> G[Desigualdades]
    F --> H[Funciones]
    G --> H
    H --> I[Funciones Polinomiales]
    H --> J[Funciones Racionales]
    I --> K[Análisis de Funciones]
    J --> K
    K --> L[Funciones Exponenciales y Logarítmicas]
    L --> M[Sucesiones y Series]
    M --> N[Matrices y Determinantes]
